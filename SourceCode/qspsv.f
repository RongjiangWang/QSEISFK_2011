...
c
      ...
      double complex y0(4,2),c0(4,2),c1(4,2),b(4,3),b0(4,3)
...
c
        call qsve2am(n,ck,0.d0,dynamic(l-1),yup,c0,2)
...
c
        ...
        if(.not.pdw(l-1))then
          ...
          c1(2,2)=(0.d0,0.d0)
          if(l-1.eq.lzr)then
            call qsve2am(n,ck,0.d0,dynamic(l-1),y0,c0,2)
            c0(2,1)=(0.d0,0.d0)
            c0(2,2)=(0.d0,0.d0)
            call qsam2ve(n,ck,0.d0,dynamic(l-1),y0,c0,2)
          endif
        endif
...
c
        call qsam2ve(n,ck,hp(l-1),dynamic(l-1),yup,c1,2)
c
...
c       determination of propagation matrix
c
        call qsve2am(n,ck,0.d0,dynamic(l),ylw,c0,2)
        pwave=cdexp(-kp(n)*ch0)
        swave=cdexp(-ks(n)*ch0)
c
c       orthonormalization of the p-sv modes
...
c
        if(.not.pup(l))then
          ...
          c1(1,2)=(0.d0,0.d0)
          if(l+1.eq.lzr)then
            call qsve2am(n,ck,0.d0,dynamic(l),y0,c0,2)
            c0(1,1)=(0.d0,0.d0)
            c0(1,2)=(0.d0,0.d0)
            call qsam2ve(n,ck,0.d0,dynamic(l),y0,c0,2)
          endif
        endif
        if(.not.svup(l))then
          c1(3,1)=(0.d0,0.d0)
...
c        c1(4,2)=c1(4,2)
c
        call qsam2ve(n,ck,-hp(l),dynamic(l),ylw,c1,2)
...
c
      ...
      key=0
      call cdgemp(coef,b,4,3,0.d0,key)
      if(key.eq.0)then
...
c
        ...
        enddo
        key=0
        call cdgemp(coef,b0,4,3,1.d-30,key)
        if(key.eq.0)then
          print *,'warning in qspsv: anormal exit from cdgemp!'
          return
        endif
...
c
        do istp=1,3
          call qsve2am(1,ck,0.d0,.true.,y(1,istp),b0(1,istp),1)
          ...
          b0(2,istp)=b(1,istp)
          b0(4,istp)=b(2,istp)
          call qsam2ve(1,ck,0.d0,.true.,y(1,istp),b0(1,istp),1)
        enddo
      endif
      return
      end
